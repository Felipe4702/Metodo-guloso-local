...
vector<int> vizinhoMaisProximo(const vector<vector<int>>& matriz, int inicio = 0) {
    ...
    for (int i = 1; i < numVertices; ++i) {
        ...
        for (int j = 0; j < numVertices; ++j) {
            if (!visitado[j] && matriz[atual][j] > 0 && matriz[atual][j] < menorDistancia) {
                menorDistancia = matriz[atual][j];
                proximo = j;
            }
        }
...
int main() {
    ...
    vector<int> solucao_atual = vizinhoMaisProximo(matriz, 0);
