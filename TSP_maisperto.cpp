...
void vizinhoMaisProximo(const vector<vector<int>>& matrizAdj, int inicio = 0) {
    ...
    for (int i = 1; i < numVertices; ++i) {
        ...
        for (int j = 0; j < numVertices; ++j) {
            if (!visitado[j] && matrizAdj[atual][j] > 0 && matrizAdj[atual][j] < menorDistancia) {
...
int main() {
    ...
    vizinhoMaisProximo(matrizAdj, 0);
