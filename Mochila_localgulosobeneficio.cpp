...
string gulosoCustoBeneficio(const vector<ItemMochila>& itens, int cap) {
    int n = itens.size();
    ...
    sort(indices.begin(), indices.end(), [&](int a, int b) {
        double cbA = (itens[a].peso > 0) ? (double)itens[a].valor / itens[a].peso : 0;
        double cbB = (itens[b].peso > 0) ? (double)itens[b].valor / itens[b].peso : 0;
        return cbA > cbB;
    });
...
int main() {
    ...
    string solucao_atual = gulosoCustoBeneficio(itens, cap);
    int melhor_valor = avaliar_mochila(solucao_atual, itens, cap);
    
    cout << "\nGuloso (Custo-Beneficio)\n";
